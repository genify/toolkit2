...
var Parser = module.exports = function(config){
    ...
    // update nej config
    var _doUpdateNEJConfig = function(config){
        var src = this.nejConfig.define;
        if (!src){
            return;
        }
        delete this.nejConfig.define;
    ...
    this.parseResource = function(config){
        // update nej config
        config = config||{};
        _doUpdateNEJConfig.call(this,config);
        // parse style list
