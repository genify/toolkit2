var util        = require('util'),
   ...
   _ExpStyle    = require('../explorer/style.js'),
...
var Parser = module.exports = function(config){
    ...
    var _doSaveResource = function(name,event,conf){
        switch(name){
            case 'style':
                if (!this.styles){
                    this.styles = new _ExpStyle({
                        file:this.file
                    });
                }
                this.styles.push(conf);
            ...
            case 'script':
                if (!this.scripts){
                    this.scripts = new _ExpScript({
                        file:this.file
                    });
                }
                this.scripts.push(conf);
            ...
            break;
            case 'template':
                var name = _gState===STATE_MODULE?'modules':'templates',
                    cache = this[name];
                if (!cache){
                    this[name] = new _ExpTemplate({
                        file:this.file
                    });
                    cache = this[name];
                }
    ...
    this.parseResource = function(config){
        ...
        // parse style list
        if (!!this.styles){
            this.styles.parse(config);
        }
        // parse script list
        if (!!this.scripts){
            this.scripts.parse(config);
        }
        // parse module list
        if (!!this.modules){
