...
pro.parse = function(config){
    ...
    if (!!err&&err.length>0){
        ...
        this._source = this._completeResource(
            this._source,config.webRoot,config.resRoot
        );
        return;
    }
    ...
    rules.forEach(function(rule){
        if (rule.type==='rule'){
            (rule.declarations||[]).forEach(function(decl){
                if ((decl.property||'').indexOf('background')>=0){
                    decl.value = this._completeResource(
                        decl.value,webRoot,resRoot
                    );
                }
            },this);
        }
 ...
 */
pro._completeResource = function(str,webRoot,resRoot){
