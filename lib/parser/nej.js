 ...
 */
var _doCompleteURI = function(uri,config){
    // for json!./a.json
    var arr = uri.split('!'),
        ret = {uri:arr[0]||''};
    if (arr.length>1){
        ...
        if (!!params.d){
            ...
            Object.keys(map).forEach(function(key){
                ...
                xlist.forEach(function(uri,index,list){
                    list[index] = _doCompleteURI(uri,opt).uri;
                });
                map[_doCompleteURI(key,opt).uri] = xlist;
            });
        }
        if (!!params.p){
...
var NEJParser = function(config){
    ...
    var _doCompleteDepList = function(config){
        ...
        this.injectorList.forEach(function(uri,index,list){
            var ret = _doCompleteURI(uri,config);
