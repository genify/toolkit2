...
exports.parseURI = function(uri,config){
    ...
    ret.uri = _path.normalize(root+brr.join('/')+ext);
    return ret;
};
/**
...
exports.parsePatch = (function(){
    ...
    // parse patch function information
    var _doParsePatchFunction = function(content){
        var ret = { args:[], riturn:'' };
        // check args list
        if (/^\s*function\s*\((.*?)\)/.test(content)){
    ...
    return function(content){
        ...
        if (!!patches){
            var ret = _doParsePatchFunction.call(
                this,content
            );
            ret.patches = patches;
            return ret;
 ...
 * @return {Void}
 */
exports.formatDependencies = function(config){
    var map = _dep.dump()||{},
        name = this.getConfig('nejInjector')||config.nejInjector;
            ...
            if (!!patch){
                ...
                if (!content){
                    var ret = [
                        _path.uri2key(patch),
                        'function(x){return x;}',
                        _path.uri2key(uri.uri)
                    ];
                    _io.cache(uri.patch,util.format(
                        '%s(%s);',name,ret.join(',')
                    ));
                }
                // cache dependency list for x.patch.js
                if (!_dep.get(patch)){
                    _dep.set(patch,[uri.uri]);
                }
            }
