 ...
 * @return {Void}
 */
var __doCheckRegularConfig = function(_root){
    var _prefix = './src/javascript/lib/regularjs/dist/';
    __doCheckValueWithDefault(
        'NEJ_REGULAR',_prefix+'regular-parser.js'
    );
    var _value = _path.path(__getConfig('NEJ_REGULAR'),_root);
    __setConfig('NEJ_REGULAR',_path.exist(_value)?_value:'');
    if (!__getConfig('NEJ_REGULAR')){
        var _value = _path.path(_prefix+'regular.js',_root);
        __setConfig('NEJ_REGULAR',_path.exist(_value)?_value:'');
    }
};
/*
...
var __doCheckConfig_DM = function(){
    ...
    __setConfig('DIR_STATIC_REG',new RegExp('([\'"])(.+?)\\1','g'));
};
/*
 * 检查混淆配置
 * @return {Void}
