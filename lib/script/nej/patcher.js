 ...
 * @param  {String} config.libRoot  - nej source root
 * @param  {String} config.webRoot  - web root path
 * @param  {Object} config.params   - other root config
...
pro.parse = function(config){
    ...
    if (!!this._source){
        ...
        args.unshift(_fs.key(),this._source);
        this._source = util.format(
            'if (%s){\n%s(%s);\n}',
            cndt,config.nejInjector,args.join(',')
        );
    }
};
/**
