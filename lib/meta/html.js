 ...
 */
pro._parseResourceOutput = function(name,content,config){
    ...
    if (!!ret.version){
        ret.version = (config.resVersionSep||'')+ret.version;
    }
    // check domain
    var domain;
    if (!!config.resDomain){
 ...
 */
pro._parseResourceEmbed = function(list,config){
    var ret = [],
    ...
    list.forEach(
        function(name){
            ...
            // generate output config
            var info = this._parseResourceOutput(
                name,content,config
            );
            // generate link code
            ret.push(util.format(
...
pro._embedResource = function(config){
    ...
    config.resVersionSep = '?';
    parser.buffer[pointer] = util.format(
        config.resWrap||'%s',
        this._parseResourceEmbed(ret,config).join('\n')
    );
...
pro.embedStyle = function(config){
    ...
    var parser = _io.getFromCache(this._uri),
        ret = this._parseResourceEmbed(
            ['t'+parser.styleConfig.outputName],
            _util.merge(config,{
                resSuffix:'.css',
                resRegexp:/<\/(textarea)>/gi,
                resInline:'<textarea name="css">%s</textarea>',
                resExternal:'<textarea name="css" data-src="%s" data-version="%s"></textarea>',
                resContentFormat:this._adjustResourceInStyle
            })
        );
    if (ret.length>0){
        parser.templateConfig.styles = ret.join('\n');
    }
};
/**
 * embed style to html file
 ...
 * @param  {Array}   config.stcDomain  - static resource domain
 * @param  {Array}   config.resDomain  - external resource domain
 * @param  {String}  config.resWrap    - resource wrap expression
 * @param  {Number}  config.maxSize    - max inline size
 * @return {Void}
 */
pro.embedScript = function(config){
    ...
    var parser = _io.getFromCache(this._uri),
        ret = this._parseResourceEmbed(
            ['t'+parser.scriptConfig.outputName],
            _util.merge(config,{
                resSuffix:'.js',
                resRegexp:/<\/(textarea)>/gi,
                resInline:'<textarea name="js">%s</textarea>',
                resExternal:'<textarea name="js" data-src="%s" data-version="%s"></textarea>'
            })
        );
    if (ret.length>0){
        parser.templateConfig.scripts = ret.join('\n');
    }
};
/**
...
pro.embedTemplate = function(config){
    _io.getFromCache(this._uri).stringify(config);
};
/**
 *
 * @param config
 */
pro.embedManifest = function(config){
