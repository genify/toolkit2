 ...
 * @return {Void}
 */
exports.cacheSpriteFile = function(file){
    var list = _gCache.sprites.list;
    if (list.indexOf(file)<0){
        list.push(file);
    }
};
/**
 * cache sprite merge result
 * @param  {Object} map - merge result map, eg. {filename:[{},{}]}
 * @return {Void}
 */
exports.cacheSpriteMergeRet = function(map){
    var ret = _gCache.sprites.ret;
 ...
 * @param  {String} root - sprite root
 * @return {Object} image will be sprited, e.g. {filename:[file1,file2,...]}
 */
exports.dumpSpriteFiles = function(root){
    var ret = {},
        def = 'sprite',
        list = _gCache.sprites.list;
    if (/\/(.+?)\/$/.test(root)){
        def = RegExp.$1;
    }
    list.forEach(function(file){
        var name = def,
            arr = file.replace(root).split('/');
        if (arr.length>1){
            name = arr.shift();
        }
        if (!ret[name]){
