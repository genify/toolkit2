    ...
    return function(event){
        ...
        if (isscript){
            ...
            if (this._state===STATE.VERSION){
                ...
                if (root!=null){
                    this.moduleConfig.root = _path.absoluteAltRoot(
                        root,path.dirname(this._file)+'/',this._options.webRoot
                    );
                    var mdl = this._options.mdlRoot;
                    event.value = util.format(
                        "<script>location.config = {root:'%s',ver:[#MODULE_VERSION#]};</script>",
                        !mdl?root:_path.wrapURI('mdl',this.moduleConfig.root)
                    );
                    return;
                }
            }
