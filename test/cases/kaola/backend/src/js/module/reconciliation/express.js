NEJ.define('{pro}module/reconciliation/express.js',['pro/widget/module','{lib}util/event.js', '{lib}util/list/module.pager.js', '{lib}util/cache/list.js', '{lib}util/ajax/dwr.js', '{pro}widget/dialog/dialog.js', '{pro}widget/toast/toast.js', '{pro}widget/window/warningWin.js'],
function(){
	var _p = NEJ.P('haitao.bm'),
        ...
        _importHtml1 = '\
            ...
            <form enctype="multipart/form-data" class="itag" action="/backend/reconciliation/express/upload" target="_blank" method="post">\
                <select class="itag" name="expressId" style="width:300px;">',
        _importHtml2 = '</select>\
		...
		_tkey = nej.e._$addHtmlTemplate('\
			<td title="${item.expressCompanyId|default:""|escape}">${item.expressCompanyId|default:""|escape}</td>\
 	...
 	_pro.__setNode = function() {
        ...
        nej.v._$addEvent(this.__export, 'click', function(){
 		...
 		this.__lopts = {
 	 			...
 	 			item: {klass: _p._$$ItemExpress},
 	 			cache: {
 	 				key: 'express',
 	 				lkey: 'express',
 	 				beanName: 'ExpressReconciliationBean',
 	 				funcName: 'listExpress',
 	 				clear: true,
 	 				klass: _p._$$CacheExpress,
                    total: 10,
        ...
        this.__dopts = {
            ...
            item: {klass: _p._$$ItemExpressDetail},
            cache: {
                key: 'express',
                lkey: 'express',
                beanName: 'ExpressReconciliationBean',
        ...
        var _optionsHmtl = '';
        for(var i=0,l=this.__config.logistics.length; i<l; i++) {
            _optionsHmtl += '<option value="'+this.__config.logistics[i].id+'">'+this.__config.logistics[i].name+'</option>'
        }
    ...
    _pro.__onDeleteItem = function(_event) {
        ...
        if(!this.__deleteDialog) {
            ...
            this.__deleteDialog = haitao.bw._$$Dialog._$allocate({
                ...
                onok: function(_dialogBody){
                    if(this.__selectMoldData.reconciliationState == 25) {
                        alert('该记录已处理，不能删除');
                        this.__deleteDialog._$hide();
                        return;
                    }

                    nej.j._$haitaoDWR('ExpressReconciliationBean', 'delete', [this.__selectMoldData.id], function(_result){
    ...
    _pro.__generateDetailMold = function(_data, _index) {
        ...
        this.__export.href = '/backend/reconciliation/express/export?reconciliationId='+_data.id;
        var _total = _data.analyseAllLineCount;

        if(_data.reconciliationState != 10) {     //导入失败不在导入详情中显示
        ...
        if(_data.reconciliationState == 5) {      //导入中
        ...
        } else if(_data.reconciliationState == 10) {      //导入失败
            _logString += "导入失败<br>"+"导入成功条数："+_data.analyseAllLineCount+"<br>导入失败条数："+_data.errors.length;
            var _errors = '<br><br>导入失败的快递单号:<br>', i=0, l=_data.errors.length;
            for(;i<l;i++){
            	_errors += _data.errors[i]+'<br>';
            }
            _logString += _errors;
        } else if(_data.reconciliationState != 0) {      //导入成功
    ...
    _pro.__onHandle = function(_event) {
        ...
        if(this.__selectMoldData.reconciliationState == 10) {
            this.__toast.__showto('数据上传失败');
            return;
        }

        if(this.__selectMoldData.reconciliationState != 15 && this.__selectMoldData.reconciliationState != 20) {
        ...
        if(!this.__handleDialog) {
            ...
            this.__handleDialog = haitao.bw._$$Dialog._$allocate({
                ...
                hideOnok: false,
                onok: function(_dialogBody){
                    nej.j._$haitaoDWR('ExpressReconciliationBean', 'handle', [this.__selectMoldData.id], function(_result){
                        if(!!_result && _result.code == 200) {
                            alert('处理成功，刷新页面！');
                            location.reload();
                        } else {
...
//        }
	...
	_p._$$CacheExpress = NEJ.C();
 	_proCache = _p._$$CacheExpress._$extend(nej.ut._$$ListCache);
    ...
    _p._$$CacheExpressDetail = NEJ.C();
    _proCacheDetail = _p._$$CacheExpressDetail._$extend(nej.ut._$$ListCache);
    ...
    _proCacheDetail.__dogetList = function(_options) {
 	 ...
 	 * 
 	 * */
 	_p._$$ItemExpress = NEJ.C();
 	_proItemExpress = _p._$$ItemExpress._$extend(nej.ui._$$ListItem);
 	
 	_proItemExpress.__init = function(_options){
 		this.__super();
 	...
 	_proItemExpress.__initNodeTemplate = function() {
 		this.__seed_html = nej.e._$addNodeTemplate('\
				<td class="xtag" title="快递公司">expressid</td>\
				<td class="xtag" title="处理状态"></td>\
				<td class="xtag" title="操作人"></td>\
				<td class="xtag" title="导入时间"></td>');
 	};

 	_proItemExpress.__doRefresh = function(_data) {
        this.__body.className = '';

 		this.__expressCompany.innerText = _data.expressName;
 	...
 	_proItemExpress.__generateDetailMold = function(_event){
    ...
    _p._$$ItemExpressDetail = NEJ.C();
    _proItemExpressDetail = _p._$$ItemExpressDetail._$extend(nej.ui._$$ListItem);
    
    _proItemExpressDetail.__init = function(_options){
        this.__super();
        var _ntmp = nej.e._$getByClassName(this.__body,'xtag'),i=0;
    ...
    _proItemExpressDetail.__doRefresh = function(_data) {
