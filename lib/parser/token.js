...
pro._reset = function(){
    ...
    this._buffer  = [];    // source cache
    this._string  = [];    // string cache
    this._strSep  = null; // string split, ' or "
    this._nospchr = null; // not space char
    this.result   = [];
};
/**
 * check white char
...
pro._white = function(c){
    ...
    if (this._state===STATE.VALUE&&
       (this._string.length>0||this._nospchr!=='=')){
        this._endValueState();
        this._begNameState();
    }
};
