...
var Processor = module.exports = function(file){
    ...
    var _doParseHtmlFiles = function(key){
        ...
        var flag = this.config.get('X_NOPARSE_FLAG'),
            ...
            resConfig = {
                ...
                processors:this.config.get('NEJ_PROCESSOR'),
    ...
    var _doPrepareMergeConfig = function(){
        var file = this.config.get('DIR_CONFIG')+'core.html';
        // check core list config
        var list = this.config.get('CORE_LIST_JS');
        if (!!list&&list.length>0){
            this.jsExplorer = new _JSExplorer({
                file:file,
                list:list
            });
        }
        // check css list config
        var list = this.config.get('CORE_LIST_CS');
        if (!!list&&list.length>0){
            this.csExplorer = new _CSExplorer({
                file:file,
                list:list
            });
        }
    ...
    var _doMergeResult = function(){
        Object.keys(this.result).forEach(function(file){
            ...
            // merge style/script/template
            inst.merge({
                htmlRoot:this.config.get('DIR_SOURCE'),
                tmplRoot:this.config.get('DIR_SOURCE_TP'),
                csDomain:this.config.get('DM_STATIC_CS'),
                jsDomain:this.config.get('DM_STATIC_JS'),
                mfDomain:this.config.get('DM_STATIC_MF'),
                mdDomain:this.config.get('DM_STATIC_MR'),
                jsCore:this.config.get('CORE_LIST_JS'),
                jsMask:this.config.get('CORE_MASK_JS'),
                csCore:this.config.get('CORE_LIST_CS'),
                csMask:this.config.get('CORE_MASK_CS'),
                freq:this.config.get('CORE_FREQUENCY')
            });
        },this);
    };
    ...
    var _doStartProcess = function(file){
        ...
        // prepare merge config
        _doPrepareMergeConfig.call(this);
