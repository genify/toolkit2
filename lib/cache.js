...
var dumpFiles2Temp = function(config){
    _log.logger.info('begin dump files ...');
    var map = {};
    ...
    _log.logger.debug('package file map -> %j',map);
    Object.keys(map).forEach(function(src){
        var dst = map[src];
        _fs.copy(src,dst,function(a,b){
            _log.logger.info('copy file %s to %s',a,b);
        });
    });
};
/**
...
var zipPackage = function(config){
    _log.logger.info('begin zip package ...');
    var cmd = [
        'java','-jar',config.zip,
        JSON.stringify(config.temp),
        JSON.stringify(config.output)
    ].join(' ');
    _log.logger.debug('do command: %s',cmd);
    exec(cmd,function(error,stdout,stderr){
        if (error){
            _log.logger.error('zip package error for reason:\n%s',error.stack);
            process.abort();
            return;
        }
        if (stdout){
            _log.logger.info(stdout);
        }
        if (stderr){
            _log.logger.error(stderr);
        }
...
var uploadToServer = function(config){
    _log.logger.info('begin upload package to web cache server ...');
    ...
    form.submit(config.api,function(err,res){
        if (err){
            _log.logger.error('upload failed for reason:\n%s',err.stack);
            clearTemp(config);
            process.abort();
            return;
        }
        _log.logger.info('package upload success');
        clearTemp(config);
        config.ondone();
    });
};
...
var clearTemp = function(config){
    _log.logger.info('clear temporary directory and files');
