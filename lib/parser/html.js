...
pro.stringify = function(config){
    this.embedTemplate(config);
    this.embedModule(config);
    var ret = [];
    this.buffer.forEach(function(it){
        if (typeof it==='string'){
            ret.push(it);
        }else{
            ret.push(_tag.stringify(it));
        }
    });
    return ret.join('\n');
};
...
pro.embedTemplate = function(config){
    // check template insert pointer
    if (this.templatePoint<0){
        return;
    }
    // check text template
    var ret = [],
        explorer = this.templates;
    if (!!explorer){
        var content = explorer.getTextTemplate();
        if (!!content){
            ret.push(content);
        }
    }
    // check style template
    var content = this.templateConfig.styles;
    if (!!content){
        ret.push(content);
    }
    // check script template
    var content = this.templateConfig.scripts;
    if (!!content){
        ret.push(content);
    }
    // check external html template
    if (!!explorer){
        var content = explorer.getHtmlTemplate(config);
        if (!!content){
            ret.push(content);
        }
    }
    ...
    if (!!content){
        this.buffer[this.templatePoint] = util.format(
            config.resWrap||'%s',content
        );
    }
 ...
 * @param config
 */
pro.embedModule = function(config){
    if (this.modulePoint<0){
        return;
    }
