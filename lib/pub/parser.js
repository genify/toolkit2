 ...
 * @return {String} 序号标识
 */
var _doGetFileKeyMap;
var _doGenFileKey = (function(){
    var _index = 1,
        _cache = {};
    _doGetFileKeyMap = function(){
        return _cache;
    };
    return function(_file){
...
var __doParseHtml = (function(){
    ...
    // check auto place holder
    var _doCheckAuto = function(_source){
        ['PG_CSS','PG_JS'].forEach(function(_name){
            if (_source.indexOf('#<'+_name+'>')>=0){
                _source = _source.replace('#<AUTO_'+_name+'>','');
            }else{
                _source = _source.replace('#<AUTO_'+_name+'>','#<'+_name+'>');
            }
        });
        return _source;
    };
    var _doWrapScript = function(_source){
        var _tag = util.format(_config.get('X_SCRIPT_WRAPPER'),'#<PG_JS>');
        return (_source||'').replace('#<PG_JS>',_tag);
    };
    return function(_file,_conf){
        ...
        for(var i=0,l=_list.length,_line,_tmp,_txg,_rot;i<l;i++){
            ...
            if (_reg20.test(_line)){
                ...
                // try to auto find nej define.js
                var _wx = !1;
                if (_tag.name=='DEFINE'||
                    _tag.name=='SCRIPT'||
                    (_wx=_reg23.test(_line[0]))){
                    if (_wx){
                        _tag = _tag||{};
                        _log.warn('see file[%s] as nej define.js',_line[0]);
                    }
                    delete _tag.name;
                    var _param = _tag.param||{};
                    delete _tag.param;
        ...
        _result.source = _doCheckAuto(
            _source.join(' ')
        );
        __doParseHtmlModule(_result);
        _result.source = _doWrapScript(
            _result.source
        );
        return _result;
    };
...
var __doParseHtmlTAG = (function(){
    ...
    return function(_line,_last,_result,_srcline){
        // comment line
        if (_line.indexOf('@')<0){
        	_log.info('comment line: %s',_line);
        	if (_config.get('X_KEEP_COMMENT')){
        	    (_last.brr||_result).push(_srcline);
        	}
        	return {};
        }
        // tag line
 ...
 * @return {Void}
 */
var __doParseHtmlERS = function(_result,_type,_src,_cache){
    ...
    _arr.push(_src);
    // flag first style/script
    if (_arr.length==1){
        // auto_pg_css / auto_pg_js
        _cache.push('#<AUTO_'+_type.toUpperCase()+'>');
    }
};
/*
 * 解析内联资源起始标签
...
var __doParseHtmlIRSEnd = (function(){
    ...
    return function(_tag,_result,_line,_cache,_file){
        ...
        if (_tag.name=='VERSION'){
            ...
            // cache code pg_css/pg_js
            var _xlist = _result[_tag.type];
            _xlist.push(_code);
            // flag first style/script
            if (_xlist.length==1){
                _cache.push('#<AUTO_'+_tag.type.toUpperCase()+'>');
            }
        }
        delete _tag.arr;
        delete _tag.type;
...
var __doParseHtmlResource = (function(){
    ...
    var _doComplete = function(_content,_file){
        var _tmpl = '%s#<:%s>%s',
            ...
            _static = _config.get('DIR_STATIC').toLowerCase();
        ...
        return _content.replace(
                ...
                function($1,$2,$3){
                    //console.log('%s relative to %s',$3,_base);
                    var _url = _doAbsolutePath($3,_base);
                    //console.log('%s && %s',_url,_static);
                    if (_url.toLowerCase().indexOf(_static)!=0){
                        return $1;
                    }
                    if (_path.exist(_url))
    ...
    var f,
        ...
        _imap = {
            ...
            regular:function(u,c){
                var rgl = _config.get('NEJ_REGULAR')||'regularjs';
                return util.format(
                    'I$(%s,%s);',
                    _doGenFileKey(u),
                    require(rgl).parse(c.replace(_regc,' '))
                );
            }
        };
    // complete path
...
var __doPrepareJS = (function(){
    ...
    return function(_result){
        ...
        var _options = {
            ...
            noconsole:_config.get('OBF_DROP_CONSOLE')
        };
        // check source map
...
var __doExlineFile = (function(){
    ...
    return function(_file,_content,_conf){
        ...
        var _md5 = _doVersionFile(_content);
        _file = _file.replace('[VERSION]',_md5);
        _log.info('output %s',_file);
        ...
        for(var x in _files){
            ...
            _files[x].source = _content
                     .replace(_reg,function($1,$2){
                                  ...
                                  if (_fobj[_flag[1]]&&!!_clink){
                                  	  ...
                                  	  if (_fobj['i'+_flag[1]]){
	                                      ...
	                                      _value = _clink.replace('#<CORE>',
	                                               _doRelativePath(
	                                                   _flag[1],_doOutputPath(x),
	                                                   _result.manifest['core_'+_flag[1]]
	                                               ));
                                  	  }
                                  }
                          ...
                          return (_flag[1]=='js'?_inject:'')+_value;
                      });
        }
    };
})();
...
var __doMergeTemplate = (function(){
    ...
    var _doMergeModule = function(_file,_data){
    	...
    	for(var i=0,l=_list.length,_src,_html;i<l;i++){
    		_src = _list[i];
    		_html = _data[_src];
    		if (!_html){
    		    _log.error('no data for external moudle[%s] in %s',_src,_file);
    		    _list[i] = '';
    		}else{
    		    _html = _html.source;
    		    _list[i] = util.format(
        		    _tmpl,_src.replace(_input,''),_html
        		);
    		}
    	}
...
var __doOutputFile = (function(){
    ...
    return function(_name,_result){
        ...
        if (!!_data.core){
            ...
            _result.manifest['core_'+_name] = _file;
...
var __doOutput = function(_result){
    ...
    _log.debug('file key map -> %j',_doGetFileKeyMap());
