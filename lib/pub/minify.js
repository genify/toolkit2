...
var __doObfuscateAPI = (function(){
    ...
    var _isCheckNode = function(_node){
        for(var x in _kmap){
            if (_node instanceof parser['AST_'+x]){
                // exclude object property with quote when mangling, eg.{"_abc":"aaaa"}
                return x!=='ObjectKeyVal'||!_node.quote;
            }
        }
        return !1;
    };
    ...
    return function(_ast,_level,_bags,_except){
        ...
        var _tw = new parser.TreeWalker(function(_node){
            ...
            //console.log(_type+':'+(_xnod.name||_xnod.value)+'-'+_xnod.mangled_name);
            _doMapVar(_xnod);
