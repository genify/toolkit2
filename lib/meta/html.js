...
var URLHANDLER = {
    ...
    cs:function(uri,config){
        config.domain = config.csDomain;
        return this._formatRSURI(
            uri,'.css',config
        );
    },
    js:function(uri,config){
        config.domain = config.rsDomain;
        return this._formatRSURI(
            uri,'.js',config
        );
    },
...
pro._formatURI = function(uri,config){
    var domain = _util.randNext(config.domain);
    if (!domain){
        // use relative path
...
pro._formatRSURI = function(name,sufix,config){
    ...
    // output
    _fs.writeAsync(file,content,config.charset);
    // format uri
