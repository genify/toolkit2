NEJ.define('{pro}module/psinventory/list.js',['pro/widget/module','{lib}util/event.js', '{lib}util/list/module.pager.js', '{pro}widget/pager/pagelist.js', '{pro}widget/window/confirm.js', '{lib}util/cache/list.js', '{lib}util/ajax/dwr.js', '{pro}widget/calendar/calendar.js'],
	function(){
		...
		_pro.__setNode = function() {
			...
			this.__indexPage = _ntmp[i++];	//当前第几页
		...
		_pro.__doSearch = function() {
			...
			var _param = [parseInt(this.__supplier.value)||-1, parseInt(this.__storage.value)||-1, parseInt(this.__shtext.value)||-1, parseInt(this.__state.value), parseInt(this.__currencyType.value), this.__contractNo.value, this.__lopts.limit, 0];
		...
		//翻页更新当前页数
		_pro.__cbSetPageIndex = function(_event) {
			this.__indexPage.innerText = '当前第'+_event.index+'页';
		};

		_pro.__afterpagershow = function() {
			this.__pageInfo.style.display = '';
		};
		...
		_proCache.__dogetList = function(_options) {
			...
			this.__param[7] = _options.offset;
		...
		_proPItem.__initNodeTemplate = function() {
			this.__seed_html = nej.e._$addNodeTemplate('\
				...
				<td title="操作">\
					<a class="xtag" href="#" target="_blank">虚拟入库</a>\
		...
		_proPItem.__doRefresh = function(_data) {
			...
			this.__link.href = '/backend/invoicing/order/mimic?id='+_data.id||'';
