        ...
        return (content||'').replace(
            reg,function($1,$2,$3){
                ...
                if ((lopt.checkDIR||!isdir)&&
                    ...
                    if (_fs.exist(arr[0])){
                        if (!isdir&&!arr[1]&&options.versionStac){
                            // check resource version with mode
                            arr = _this._checkResVersion(
                                arr[0],options
                            );
                        }
...
/**
 * check static resource version with mode
 *
 * @private
 * @param  {String} uri - file path
 * @param  {String} mode - static version mode
 * @return {Array}  static resource file path and version
 */
pro._checkResVersion = function(uri,options){
    // use version mode for static resource
    var base = path.basename(uri),
        ext = path.extname(base);
    // generate version with mode
    var ver = _util.md5(_fs.raw(uri)),
        ret = _util.version(
            options.versionStacMode,{
                RAND:_util.increment(),
                VERSION:ver,
                FILENAME:base.replace(ext,'')
            }
        );
    // copy resource if not equal
    var name = ret.file+ext,
        file = path.dirname(uri)+'/'+name;
    if (name!==base&&!_fs.exist(file)){
        _fs.copy(uri,file,function(src,dst){
            this.emit('debug',{
                data:[src,dst],
    ...
    // generate file and version
    var arr = [file];
    if (!!ret.version){
        arr.push(ret.version);
    }
    return arr;
};
/**
