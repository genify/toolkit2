...
pro.getDependencies = function(config){
    ...
    // check ignore entry
    if (config.ignoreEntry&&this._isEntry){
...
pro._completeURI = function(uri,config){
    ...
    if (!!uri){
        var arr = uri.split('?');
        arr[0] = this._super(arr[0],config);
        uri = arr[0];
    }
    // check nej define.js
    if (this._isNEJDefine){
        ...
        _nej.supportNEJCompatible();
        // cache dep config
        dep = _nej.cacheDependencies(dep,config);
        // adjust nej dependency
        uri = _nej.NEJ_DEFINE_FILE;
