...
var util        = require('util'),
   ...
   _ExpScript   = require('../explorer/script.js'),
   _ExpTemplate = require('../explorer/template.js'),
...
var Parser = module.exports = function(config){
    ...
    // private cache
    var _gState,_gOption,_gBuffer,
        _gStyles,_gScripts,_gModules,_gTemplates,
        _gNEJConfig,_gStyleConfig,_gScriptConfig,_gModuleConfig,
        _gNoCompressPoint,_gIgnorePoint,_gManifestPoint,
        _gStylePoint,_gScriptPoint,_gVersionPoint,_gTemplatePoint;
    ...
    var _doSaveResource = function(name,event,conf){
        switch(name){
            ...
            break;
            case 'template':
                var key = _gState===STATE_MODULE?'modules':'templates',
                    cache = this[key];
                if (!cache){
                    this[key] = new _ExpTemplate({
                        file:this.file
                    });
                    cache = this[key];
                }
    ...
    var _onScript = (function(){
        ...
        return function(event){
            ...
            if (!!conf.src&&issp){
                ...
                if (ishd||_reg1.test(ropt.uri||'')){
                    if (ishd){
                        _gState = STATE_NULL;
                    }
                    this.nejConfig.define = ropt.uri;
                    ropt.entry = !1;
                    ropt.uri = _path.absolute(
                        '../define.js',
    ...
    var _onTag = function(event){
        ...
        if (_gState===STATE_MANIFEST&&attrs.name==='html'){
            _gState = STATE_NULL;
    ...
    this.update = function(file,options){
        var content = _doDumpContent.call(
            this,file,config.charset
        );
    ...
    // processors     pre processor for resource
    this.parseResource = function(config){
        // update no core flag
        _doUpdateNoCoreFlag.call(
            this,config.noCoreFlag
        );
        var opt = {
        ...
        // parse style list
        if (!!_gStyles){
            _gStyles.parse(config);
        }
        // parse script list
        if (!!_gScripts){
            _gScripts.parse(config);
        }
        // parse module list
        if (!!this.modules){
            this.modules.parse(config);
        }
        // parse template list
        if (!!this.templates){
            this.templates.parse(config);
        }
    };
    ...
    //    });
    //};
    //// merge css/script/template
    //this.splitCore = function(config){
    //
    //};
    //// get style list in page
    //this.getStyles = function(){
    //
    //};
    //// get script list in page
