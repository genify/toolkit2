...
exports.cache = (function(){
    var _downloading = {};
    var _doFormatContent = function(file,content,callback){
        ...
        _memory.setFileContent(file,event.value);
    };
    return function(file,callback,charset){
        // check cache
        var content = _memory.getFileContent(file);
        if (content!=null){
            callback({
                fromCache:!0,
                value:content
        ...
        if (!_path.isURL(file)){
            // local file
            var content = (this.read(file,charset)||[]).join('\n');
            _doFormatContent(file,content,callback);
        }else if (!_downloading[file]){
            // remote file
            _downloading[file] = !0;
            this.download(
                file+'?v='+(+new Date),function(content){
                    _doFormatContent(file,content,callback);
                    delete _downloading[file];
                }
            );
        }
    };
