...
pro.init = function(config){
    ...
    // update merge config
    _log.logger.info('prepare core and mask config');
...
pro._afterResPrepared = function(){
    ...
    // resort resource
    _log.logger.info('sort page by module dependency');
    this._result.sort();
    // merge resource
    _log.logger.info('split core from page');
    this._result.merge({
    ...
    // check deploy finish
    _io.ondone(function(){this.emit('done');}.bind(this));
};