NEJ.define(['{lib}util/chain/chainable.js', '{lib}util/cache/list.js',
    ...
    '{pro}components/modal/feesChange/feesChange.js',
    '{pro}widget/dialog/dialog.js',
    '{lib}util/ajax/dwr.js',
    '{pro}widget/calendar/calendar.js',
    'pro/widget/module'
], function(_q, _l, _j, _u, _e, BaseComponent, FeesChangeModal) {
    var variousfees = BaseComponent.extend({
        template : document.getElementById('variousfees').value,
        data:{},
        text:['国际','转关','佣金'],
        ...
        getData: function() {
            var canbeReturn = this.checkData();
            if(!!canbeReturn.error) {
                // alert(canbeReturn.error);
                return canbeReturn;
            } else {
                return canbeReturn;
            }
        ...
        checkData: function() {
            var item, rtobj = {}, error = [];
            for(var i=0, l=this.data.dataAry.length; i<l; i++) {
                item = this.data.dataAry[i];
                rtobj[item.pre+'StateValue'] = item.State;
                rtobj[item.pre+'MaintenanceStatusValue'] = item.MaintenanceStatus;

                item.currencyError = item.exchangeError = item.paymentError = false;
                if(item.State == 0) {   //无费用
                    rtobj[item.pre+'CurrencyTypeValue'] = 0;
                    rtobj[item.pre+'ExchangeRate'] = null;
                    rtobj[item.pre+'PaymentAmount'] = null;
                } else {
                    //TODO  数据校验
                    //是否填写校验
                	/** JIRA#KJ-6153:海淘采购单创建必输项调整
                    if(Number(item.CurrencyType) <= 0) {
                        error.push(this.text[i]+'费用的币种为必填！');
                        item.currencyError = true;
                    }
                    **/

                    if(!!item.ExchangeRate && (isNaN(item.ExchangeRate) || Number(item.ExchangeRate)<0)) {
                        error.push(this.text[i]+'费用的汇率必须为大于0的数字');
                        item.exchangeError = true;
                    }

                    if(!!item.PaymentAmount && (isNaN(item.PaymentAmount) || Number(item.PaymentAmount)<0)) {
                        error.push(this.text[i]+'费用的支付金额必须为大于0的数字');
                        item.paymentError = true;
                    }

                    rtobj[item.pre+'CurrencyTypeValue'] = item.CurrencyType;
                    rtobj[item.pre+'ExchangeRate'] = this.fixedNumber(item.ExchangeRate);
                    rtobj[item.pre+'PaymentAmount'] = this.fixedNumber(item.PaymentAmount);
                }
            }
        ...
        checkItem: function(item, index) {
            ...
            if(item.State == 1) {   //有费用
                if(Number(item.CurrencyType) <= 0) {
                    error.push(this.text[index]+'费用的币种为必填！');
                    item.currencyError = true;
                }

                if(!!item.ExchangeRate && (isNaN(item.ExchangeRate) || Number(item.ExchangeRate)<0)) {
                    error.push(this.text[index]+'费用的汇率必须为大于0的数字');
                    item.exchangeError = true;
                }

                if(!!item.PaymentAmount && (isNaN(item.PaymentAmount) || Number(item.PaymentAmount)<0)) {
                    error.push(this.text[index]+'费用的支付金额必须为大于0的数字');
                    item.paymentError = true;
                }
            }
            return error;
        },
        modify: function(info, index) {
            var arg = {
                    purchaseState: this.data.state, 
                    feesData: info, 
                    title:this.text[index]+'费用修改', 
                    index:index,
                    purchaseId:Number(_q('.u-page-title')._$attr('data-ponumber'))
                };
            var feesModal = new FeesChangeModal({data:arg});
            feesModal.$on('confirm', function(_data, _index) {
                console.log(-1);
            })
        },
        setDataItem: function(index) {
...
//            exchangeRate: _q('#po-exchangerate'),
    ...
    _pro.__regEvents = function(_options){
        ...
        _q('#financeCheckBtn')._$on('click',this.__financeCheck._$bind(this) );   //财务核对

        _q('#cancelPageBtn')._$on('click', function() {
            // 取消回到上一个页面
            history.go(-1);
        });
        ...
        this.__poHeader.purchaseWay._$on('change', function(){
            //采购方式在更改后去除警告信息
            this.__poHeader.purchaseWay._$delClassName('f-alert');
        }._$bind(this));
    ...
    _pro.__initRegularMod = function() {
        //处理国际费用等功能
        this.__variousfees = new variousfees({data:variousfeesData});
        this.__variousfees.$inject('#variousfeesbox');
    }
    ...
    _pro.__normalChange = function(_event){
        ...
        if (_event.target.className.indexOf('poqty-i')>=0){
            ...
            var reg = /^([1-9]\d*|0)\.\d{1,4}$|^([1-9]\d*|0)$/;
            if (!reg.test(_val)){
                alert('采购单价需为最多4位小数的非负数');
                _q(_event.target)._$addClassName('f-alert');
                _event.target.focus();
            }else{
                if (this.__maxValueOverflow(_val)) {
    ...
    _pro.__updateStastics = function(_event){
        ...
        this.__poHeader.goodsAmount._$text( _u._$fixed(_counter.amt, 4) );
        ...
        _row._$children('.lineamount', true)._$text( !!_line ? _u._$fixed(_line.qty * _line.price, 4) : '' );
    ...
    _pro.__validateSavingData = function(){
        ...
        if (this.__poHeader.purchaseWay._$val() < 0){
            _message.push('请填写采购方式');
            this.__alert(this.__poHeader.purchaseWay);
        }
        ...
        //费用校验
        var variousfees = this.__variousfees.getData();
        if(!!variousfees.error) {
            _message.push(variousfees.error);
        }
    ...
    _pro.__save = function(_event){
        ...
        if ( _header.id <= 0 ) {
            _params = [_header, 0];
            if ( fromTrackTask ) {
                var _auditId = window.location.search.split('=')[1];
                _params[1] = _auditId/1;
            }
        }
            ...
            _params, function(_data){
                ...
                if ( fromTrackTask ) {
                    ...
                    var _refer = document.referrer,
                        ...
                        _index = _refer.lastIndexOf('&t=');
                    if ( _index != -1 ) {
                        window.location = _refer.replace(/&t=.*$/g, '&t=' + _time);
                    } else {
                        window.location = document.referrer + '&t=' + _time;
                    }
                    return;
                }
    ...
    _pro.__buildPOHeader = function(){
        ...
        _header.storageName = _hobj.warehouse._$children('option:selected')._$text();
...
//        _header.exchangeRate = parseInt(Number(_hobj.exchangeRate._$val())*1000000)/1000000;
        ...
        _header.purchaseWayValue = Number(_hobj.purchaseWay._$val());
        ...
        var variousfees = this.__variousfees.getData();
        if(!variousfees.error) {
            nej.u._$merge(_header, variousfees);
        }

        _header.auditId = window.auditId || 0;
    ...
    _pro.__financeCheck = function(){
        this.__setBtnStatus('disable');
        //入库拒绝对话框
        if (this.__finCheckDialog){
            this.__finCheckDialog._$recycle();
            this.__finCheckDialog = null;
        }
        var _ponumber = {poNumber: this.__poHeader.ponumber._$attr('data-ponumber') };
        _j._$add('jst-finance');
        var _html = _j._$get('jst-finance', _ponumber);

        this.__finCheckDialog = haitao.bw._$$Dialog._$allocate({
            parent: document.body,
            title: '提示',
            align: 'middle middle',draggable: true,
            content: _html,
            ...
            onok: function(_dialogBody){
                ...
                var _poNumber = Number(this.__poHeader.ponumber._$attr('data-ponumber'));
                var _remark = _q('.confirm-memo', _dialogBody)._$val();
                nej.j._$haitaoDWR('InvoicingBean', 'checkFinance',
                    [_poNumber, _remark], function(_data){
                        if (!_data) {
                            _q('.alert-text', _dialogBody)._$text('提交失败，请重试');
                            _q('.ok', _dialogBody)[0].removeAttribute('disabled');
