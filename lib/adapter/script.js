...
pro.parse = function(config){
    ...
    this._generateOutput(config.sourcemap);
};
/**
 * dump result after uglify
 * @returns {Object} parse result, eg. {bags:{},code:{},sourcemap:{}}
 */
pro.dump = function(){
    return this._result;
};
...
pro._warn = function(){
    var msg = uglify.
        string_template.apply(uglify,arguments);
    this.emit('warn',{ message:msg });
};
/**
...
pro._mergeCodeAndToAST = function(map,codes){
    ...
    Object.keys(map).forEach(
        function(file){
            var list = map[file];
            list.forEach(function(name,index,list){
                var code = codes[name];
                list[index] = code||'';
                if (!code){
                    this.emit('warn',{
    ...
    // merge ast
    Object.keys(this._asts).forEach(function(file){
        var ast = ret[file];
        if (!this._ast){
            this._ast = ast.clone();
            return;
        }
        this._ast.body = this._ast.body.concat(ast.body);
...
pro._mangleAPI = (function(){
    ...
    return function(level,bags,except){
        ...
        this._result.bags = _ret;
    };
})();
/**
 * generate output content
 * @private
 * @param  {Object} sourcemap - source map output config
 * @param  {String} sourcemap.root - root of source map file
 * @return {Void}
 */
pro._generateOutput = function(sourcemap){
    var config = {
        max_line_len:10000
    };
    // output code
    var root = (sourcemap||{}).root;
    this._result.code = {};
    if (!!root){
    ...
    Object.keys(this._asts).forEach(
        function(file){
            ...
            if (!!root){
                ...
                config.source_map = new uglify.SourceMap({
            ...
            // output code
            var stream = new uglify.OutputStream(config);
            ast.print(stream);
            var code = ''+stream;
            // check source map
            if (!!root){
