var util     = require('util'),
   ...
   _memory   = require('../util/memory.js'),
...
var ResScript = module.exports = function(config,events){
    ...
    this.parse = function(config){
        ...
        if (this.url===_const.NEJ_DEFINE_URL){
            if (!_memory.getFileContent(this.url)){
                var file = path.dirname(__filename)+'/injector.js';
                _memory.setFileContent(this.url,_fs.read(file).join('\n'));
            }
            return;
        }
    ...
    this._parseURL = function(config){
        _fs.cache(this.url,function(event){
            if (!event.fromCache){
                event.value = this._parseNEJFile(
