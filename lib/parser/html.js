...
var TRANSFORM = {
    style:function(event,config){
        // check style merge
        if (this._state===STATE.MERGE){
            this._saveMergeStyle(config);
            event.value = '';
            return;
        }
        // common style
    ...
    script:function(event,config){
        // check script merge first
        if (this._state===STATE.MERGE){
            this._saveMergeScript(config);
            event.value = '';
            return;
        }
        // common script
        ...
        if (this.scriptPoint<0){
            this._doUpdateScriptPoint(event.buffer);
        }
        event.value = '';
    },
    template:function(event,config){
        ...
        // last template insert config
        var conf = this.nejTemplates[
            this.nejTemplates.length-1
        ];
        if (!conf){
            conf = {
                pointer:event.buffer.length
            };
            this.nejTemplates.push(conf);
        }
        // save nej template
        if (!conf.explorer){
            conf.explorer = new (require('../explorer/template.js'))(
                _util.merge(this.getLogger(),{
                    file:this._file
                })
            );
        }
        conf.explorer.push(config);
        // if (!this.templates){
    ...
    MERGE:{
        beg:function(event){
            if (this._state!==STATE.MERGE){
                ...
                event.buffer.push(STATE.FLG_MERGE);
            }
        },
        end:function(event){
            if (this._state===STATE.MERGE){
...
// bodyPoint        body start tag pointer in buffer
...
pro.parse = function(config){
    ...
    if (!!root){
        var func = function(explorer){
            if (!!explorer){
                explorer.updateModuleRoot(root);
            }
        };
        func(this.modules);
        this.nejTemplates.forEach(function(conf){
            func(conf.explorer);
        });
        // [
        //     this.modules,
    ...
    [this.styles,this.scripts,this.modules].forEach(func);
    this.nejTemplates.forEach(function(conf){
        func(conf.explorer);
    });
};
...
pro._reset = function(config){
    ...
    this.scriptPoint    = -1;
    this.modulePoint    = -1;
    // support multiple template for one page
    //this.templates      = null;
 ...
 * @private
 * @param  {Object} config - config object
 * @param  {String} config.uri  - external resource uri
 * @param  {String} config.text - inline resource text
 * @return {Void}
 */
pro._saveMergeStyle = function(config){
    var arr = this.mergeConfig,
        conf = arr[arr.length-1]||{};
    if (!conf.styles){
        conf.styles = new (require('../explorer/style.js'))(
            _util.merge(this.getLogger(),{
                file:this._file
            })
        );
    }
    conf.styles.push(config);
};
/**
 * save merge script
 * @private
 ...
 * @return {Void}
 */
pro._saveMergeScript = function(config){
    var arr = this.mergeConfig,
        list = (arr[arr.length-1]||{}).scripts||[],
        pathRoot = path.dirname(this._file)+'/';
    // push inline script
    if (!!config.text){
        var uri = pathRoot+_util.increment()+'.js';
        list.push(uri);
        this.emit('debug',{
            data:[uri],
            message:'cache script %s'
        });
        _io.cache(uri,config.text);
    }
    // push external script
    if (!!config.uri){
        var uri = _path.absoluteAltRoot(
            (config.uri||'').split(/[?#]/)[0],
            pathRoot,this._options.webRoot
        );
        list.push(uri);
        this.emit('debug',{
            data:[uri],
            message:'load script %s'
        });
        _io.get(uri);
    }
};
...
/**
 * update script insert pointer
 *
 * @private
 * @param  {Array} buffer - html code buffer
 */
pro._doUpdateScriptPoint = function (buffer) {
    var point = buffer.length;
    // check module insert point, ensure insert module in body
    if (this.bodyPoint>=0&&point>this.bodyPoint){
        buffer.push('');
        this.modulePoint = point;
    }
    this.scriptPoint = buffer.length;
};
/**
        ...
        if (isscript){
            ...
            if (this._state===STATE.VERSION){
                ...
                if (root!=null){
                    ...
                    this.emit('debug',{
                        data:[file,this._file],
                        message:'find nej module root %s at %s'
                    });
                    this.moduleConfig.root = file;
                    event.value = util.format(
                        "<script>location.config = {root:'%s',mode:%s,ver:%s};</scr"+"ipt>",
                        _path.wrapURI('mdl',file),_util.MODULE_VERSION_HOLDER,_path.wrapVersion(file)
                    );
                    return;
                }
            }
...
pro._onTag = function(event){
    ...
    if (this._state===STATE.MANIFEST&&name==='html'){
    ...
    // check body tag for module insert point
    if (name==='body'){
        if (!tag.closed){
            // record body start tag pointer
            this.bodyPoint = event.buffer.length-1;
        }else{
            if (!this.modules){
                // reset point if no module to insert
                this.modulePoint = -1;
            }else if (this.modulePoint<0){
                // insert module before </body>
