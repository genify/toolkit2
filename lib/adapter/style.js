...
pro.parse = function(config){
    ...
    if (!!err&&err.length>0){
        ...
        this._source = this._completeResource(
            this._source,config.webRoot,
            config.resRoot,config.sptRoot
        );
        return;
    }
        ...
        (rule.declarations||[]).forEach(function(decl){
            if ((decl.value||'').indexOf('url(')>=0){
                decl.value = this._completeResource(
                    decl.value,webRoot,resRoot,sptRoot
                );
            }
        },this);
    },this);
 ...
 * @param  {String} resRoot - resource root path
 * @param  {String} sptRoot - sprite root path
 * @return {String} string after complete resource path
 */
pro._completeResource = function(str,webRoot,resRoot,sptRoot){
